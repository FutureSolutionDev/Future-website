...
export function Navbar() {
    ...
    return (
        <nav className="fixed w-full z-50 top-0 left-0 bg-bg-dark/80 backdrop-blur-md border-b border-white/10">
            <div className="container mx-auto px-4 h-20 flex items-center justify-between">
                {/* Logo */}
                <Link href="/" className="text-2xl font-bold text-white tracking-tighter">
                    Future<span className="text-primary-blue">Solutions</span>
                </Link>
                ...
                <div className="hidden md:flex items-center space-x-8">
                    {navLinks.map((link) => (
                        <Link
                            key={link.href}
                            href={link.href}
                            className="text-text-muted hover:text-cyan-glow transition-colors text-sm font-medium"
                        >
                            {link.label}
                        </Link>
                    ))}
                        ...
                        <div className="container px-4 py-8 flex flex-col space-y-4">
                            {navLinks.map((link) => (
                                <Link
                                    key={link.href}
                                    href={link.href}
                                    className="text-lg font-medium text-white hover:text-primary-blue"
